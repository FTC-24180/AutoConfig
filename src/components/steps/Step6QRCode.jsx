  ...
  return (
    ...
    >
      <div className="space-y-6 pb-6">
                  ...
                  <button
                    ...
                    className={`p-3 rounded-lg border-2 transition-all touch-manipulation min-h-[60px] ${
                      selectedMatchIndex === index
                        ...
                        : 'border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-900 active:bg-gray-50 dark:active:bg-slate-800'
                    }`}
                  >
                    <div className="text-left">
                      <div className="font-bold text-gray-800 dark:text-gray-100">Match #{matchData?.number}</div>
                      <div className="text-xs text-gray-600 dark:text-gray-300 flex items-center gap-1">
                        <AllianceIcon 
                          alliance={alliance?.color} 
                          className={`w-3 h-3 ${alliance?.color === 'red' ? 'text-red-600' : 'text-blue-600'}`}
                        />
        ...
        {/* QR Code Display */}
        {currentMatchConfig ? (
          <div className="bg-white dark:bg-slate-900 p-6 rounded-lg border-2 border-gray-200 dark:border-slate-700 flex flex-col items-center">
            <div className="mb-3 text-center">
              <span className="inline-block px-3 py-1 bg-indigo-100 dark:bg-indigo-950/50 text-indigo-800 dark:text-indigo-200 rounded-full text-sm font-semibold">
        ...
        {config.matches && config.matches.length > 0 && (
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-4">
            <h4 className="font-semibold text-gray-900 dark:text-gray-100 mb-2">All Matches</h4>
            <div className="space-y-2">
              {config.matches.map((item, index) => {
                ...
                return (
                  <div 
                    key={index} 
                    className={`bg-gray-50 dark:bg-slate-800 rounded p-3 text-sm transition-all cursor-pointer touch-manipulation ${
                      selectedMatchIndex === index ? 'ring-2 ring-indigo-500' : ''
                    }`}
                    onClick={() => setSelectedMatchIndex(index)}
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-bold text-gray-800 dark:text-gray-100">Match #{matchData?.number}</span>
